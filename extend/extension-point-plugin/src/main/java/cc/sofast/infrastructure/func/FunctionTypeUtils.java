...
public final class FunctionTypeUtils {
    ...
    public static Type discoverFunctionType(Object function, String functionName, GenericApplicationContext applicationContext) {
        if (function instanceof TenantFunctionRegistration) {
            return ((TenantFunctionRegistration) function).getType();
        }
        if (applicationContext.containsBean(functionName + TenantFunctionRegistration.REGISTRATION_NAME_SUFFIX)) { // for Kotlin primarily
            TenantFunctionRegistration fr = applicationContext
                    .getBean(functionName + TenantFunctionRegistration.REGISTRATION_NAME_SUFFIX, TenantFunctionRegistration.class);
            return fr.getType();
        }
        ...
        if (applicationContext.containsBean("&" + functionName)) {
            Class<?> objectType = applicationContext.getBean("&" + functionName, FactoryBean.class)
                    .getObjectType();
            return discoverFunctionTypeFromClass(objectType);
        }

        Type type = discoverFunctionTypeFromClass(function.getClass());
        if (beanDefinitionExists) {
            Type t = getImmediateGenericType(type, 0);
    ...
    private static void assertSupportedTypes(Type type) {
        ...
        Assert.isTrue(Supplier.class.isAssignableFrom(candidateType)
                ...
                || TenantFunctionRegistration.class.isAssignableFrom(candidateType)
    ...
    private static Type extractReactiveType(Type type) {
        if (type instanceof ParameterizedType && TenantFunctionRegistration.class.isAssignableFrom(((Class<?>) ((ParameterizedType) type).getRawType()))) {
            type = getImmediateGenericType(type, 0);
            if (type instanceof ParameterizedType) {
                type = getImmediateGenericType(type, 0);
            }
